...
export default function WORKERS() {
    ...
    const canView = FIND_PERMIT(permits, 'workers', 1);
    //const cancreate = FIND_PERMIT(permits, 'workers', 2); // this is invite
    const canEdit = FIND_PERMIT(permits, 'workers', 3); // this is asign
    const canDelete = FIND_PERMIT(permits, 'workers', 4); // this is set activo 1 | 0
    ...
    function manage(data, object) {
        ...
        ALERT_WAIT(lang);
        if (object.id) {
            SERVICE_WORKERS.update(object.id, formData)
                .then(response => {
    ...
    function saveRoles(id) {
        ...
        ALERT_WAIT(lang);
        SERVICE_WORKERS.update(id, formData)
            .then(response => {
                if (response.data === 'OK') ALERT_SUCCESS(lang);
                else if (response.data === 'NO PERMIT') ALERT_NO_PERMIT(lang);
                else ALERT_ERROR(lang);
    ...
    return (
        <>
            <Row className="text-center" style={{ width: '100%' }}>
                <h3>{'WORKERS ASIGNAMENTS'} <BTN_HELP
            ...
            {canEdit ? 'i can asign!' : ''}
            {canView ?'i can see!' : <NON_IDEAL_STATE type="permit" />}
        </>
    );
}
