        ...
        <Drawer.Body className={utilities ? utilities.theme : 'light'} >
            {page.map(it => {
                return <>
                    ...
                    onClick={() => {setFocus(it.focus)}}
                    style={it.focus == focused ? {color: 'violet'}: {}}
                   /></p>
                    ...
                    <p className='text-justify'>{it.content}</p>
                    {it.list ? <ul>{it.list.map(l => <li>
                        <label className='fw-b'>{l.subtitle}: </label><label>{l.text}</label>
                    </li>)}</ul> : ''}
