...
export default function BTN_DOWNLOAD(props) {
    ...
    let text_content = <h6 className='fw-b' style={{ paddingTop: '12px' }}>{text}</h6>;
    let btnCols = sm ? [5, 19] : [8, 16];
    return (
        sm ? <Button color={color || 'red'} appearance='ghost' onClick={onClick} size="xs">
            {icon}  {text}
        </Button>
